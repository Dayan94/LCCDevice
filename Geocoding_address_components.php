...
class Geocoding
{
    ...
    public function getAddress($latitude, $longitude)
    {
        ...
        $result = [];
        $length_of_address_components = count(json_decode($addressData)->results[0]->address_components);
        for($i = 0; $i < $length_of_address_components  ; $i++)
        {
            $result[$i] = json_decode($addressData)->results[0]->address_components[$i]->long_name;
        }
        return ($result);
    }
